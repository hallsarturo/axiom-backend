...
fs.createReadStream(csvFilePath)
    ...
    .on('end', async () => {
        try {
            ...
            logger.info(`Inserted ${categoryArray.length} categories.`);
            // If you want to log the actual array:
            logger.info(JSON.stringify(categoryArray));
        } catch (err) {
            logger.error('Bulk insert error:', JSON.stringify(err));
        }
    });
