...
try {
    ...
    // If you want to log the error object:
    logger.error(
        'File does not exist or is not accessible: ' + JSON.stringify(err)
    );
}
...
function writePostsToFile(outputPath) {
    try {
        ...
        logger.info(`Posts written to ${outputPath}`);
    } catch (err) {
        logger.error('Error writing posts to file: ' + JSON.stringify(err));
    }
}
