...
// logger.info('TWILIO_AUTH_TOKEN:', authToken ? 'exists' : 'MISSING');
...
export async function createVerification(mobilePhone) {
    ...
    logger.info(JSON.stringify(verification.status));
}
...
export async function verificationCheck(otpCode, mobilePhone) {
    ...
    logger.info(JSON.stringify(verificationCheck.status));
